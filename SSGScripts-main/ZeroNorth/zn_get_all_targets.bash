...
tgts=$(echo "$result" | jq -r '.[0][] | .id+"|"+.data.name+"|"+.data.environmentType+"|"+(if (.data.tags != null) then (.data.tags|join(",")) else "" end)')
...
if [ ! "$NO_HEADERS" ]; then
    if [ "$INCLUDE_POLICIES" ]; then
        if [ "$INCLUDE_JOBS" ]; then
            echo "Tenant|TargetID|TargetName|TargetType|TargetTags|Status|PolicyId|PolicyName|Scanner|job_date"
        else
            echo "Tenant|TargetID|TargetName|TargetType|TargetTags|Status|PolicyId|PolicyName|Scanner"
        fi
    else
        echo "Tenant|TargetID|TargetName|TargetType|TargetTags"
    fi
fi

# Iterate through the Targets.
while IFS="|" read tgt_id tgt_name tgt_type tgt_tags
do
    # The POLICIES option not specified.
    if [ ! "$INCLUDE_POLICIES" ]; then
        # Print the App/Target-level data.
        echo "$cust_name|$tgt_id|$tgt_name|$tgt_type|$tgt_tags"
        continue
    fi
    ...
    if [ $pol_count -eq 0 ]; then
        echo "$cust_name|$tgt_id|$tgt_name|$tgt_type|$tgt_tags|No Policies"
        continue
    fi
    ...
    do
        ...
        if [ ! "$INCLUDE_JOBS" ]; then
            echo "$cust_name|$tgt_id|$tgt_name|$tgt_type|$tgt_tags|$pol_status|$pol_id|$pol_name|$pol_scenario"
        else
            # Iterate through the jobs history.
            while read job_date
            do
                echo "$cust_name|$tgt_id|$tgt_name|$tgt_type|$tgt_tags|$pol_status|$pol_id|$pol_name|$pol_scenario|$job_date"
            done <<< "$jobs"
        fi
    done <<< "$policies"
